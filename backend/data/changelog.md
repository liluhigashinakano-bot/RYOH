...
- **追加注文の金額が概算伝票に反映されない問題を修正（改善）**
  - 「条件を追加した金額」画面で追加したアイテムが、概算伝票PDFに正しく反映されていない問題
  - モーダル内で計算した `estimateGrandTotal` の値を使用して `adjustment` を計算
  - `adjustment = estimateGrandTotal - grandTotal` で正確な追加金額をAPI に渡す
  - コミット: `c95fe93f` → `61cead0c`（改善版）

### UI改善
- **概算伝票: 値引き機能の配置変更**
  - 「💴 値引きして発行」ボタンを削除
  - 選択画面と追加注文画面の概算合計金額をクリック可能に
  - 金額をクリック → 値引きモーダルが開く
  - 選択画面と追加注文画面の金額左に ✏️ マークを追加してクリック可能であることを表示
  - UX: 値引きを「発行方法の選択肢」から「金額の編集機能」に変更
  - コミット: `88001965` → `f48502ed` → `e5fc2323` → `8a70d324`（鉛筆マーク追加）

- **担当者入力をプルダウン選択に統一**
  - 伝票削除時（TicketDeleteModal）: プルダウン選択 ✅
  - 開始時刻修正フォーム: テキスト入力 → プルダウン選択に変更（実装後 React Hook error で問題発生）
  - オーダー削除/編集パネル: テキスト入力 → プルダウン選択に変更（実装後 React Hook error で問題発生）
  - 各セクション内で `useQuery` を使用して staffMembers を取得（スコープエラー解決）
  - 店舗所属の社員・アルバイトを優先表示、他店舗従業員も選択可能に
  - コミット: `1ba298f4` → `d932060a` → `63b963fa`（最終実装）

### バグ修正
- **React Hook error #310 を修正**
  - 開始時刻修正フォームと オーダー削除/編集パネルで `useQuery` を IIFE 内で呼び出していた
  - React Hook ルール違反（条件付き Hook 呼び出し）により、ticket 削除時に "エラーが発生しました" が発生
  - 修正: `useQuery` IIFE をテキスト入力に戻す（コミット: `a126a0dd`）
  - **次のステップ**: 各セクションで独立した useQuery を使用し、staff list をトップレベルで取得して props で渡すアーキテクチャに変更予定
